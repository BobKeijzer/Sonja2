...
class AddAgendaItemInput(BaseModel):
    ...
    prompt: str = Field(description="De opdracht/prompt die Sonja moet uitvoeren op het geplande tijdstip")
    ...
    mail_to: list[str] = Field(default_factory=list, description="E-mailadressen waar het resultaat naartoe gestuurd moet worden")
...
class AddAgendaItemTool(BaseTool):
    ...
    description: str = (
        ...
        "Geef title, prompt (wat Sonja moet doen), type (once of recurring), schedule (ISO datetime of cron), "
        "en optioneel mail_to (e-mailadressen voor het resultaat). Bij 'once' wordt het item na uitvoering verwijderd."
    )
    args_schema: Type[BaseModel] = AddAgendaItemInput

    def _run(self, title: str, prompt: str, type: str, schedule: str, mail_to: list[str] | None = None) -> str:
